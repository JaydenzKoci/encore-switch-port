...
//----------------------------------------------------------------------------------
// LibNx Specific Functions Definition
//----------------------------------------------------------------------------------

// Show nx virtual keyboard
bool ShowNxKeyboard(char* text, int textSize, const char* title, const char* placehold, const char* ok, const char* initial, SwkbdType type)
{
    SwkbdConfig kbd;
    ...
    rc = swkbdShow(&kbd, text, textSize);
    if (R_FAILED(rc)) return false;
...
bool GuiTextBox(Rectangle bounds, char *text, int bufferSize, bool editMode)
{
    ...
    if ((state != STATE_DISABLED) && !guiLocked && !guiSliderDragging)
    {
        ...
        if (editMode)
        {
            ...
            pressed = ShowNxKeyboard(text, bufferSize, "", "", "Enter", text, SwkbdType_QWERTY);
...
bool GuiValueBox(Rectangle bounds, const char *text, int *value, int minValue, int maxValue, bool editMode)
{
    ...
    if ((state != STATE_DISABLED) && !guiLocked && !guiSliderDragging)
    {
        ...
        if (editMode)
        {
            ...
            pressed = valueHasChanged = ShowNxKeyboard(textValue, RAYGUI_VALUEBOX_MAX_CHARS, "", "", "Enter", textValue, SwkbdType_NumPad);
